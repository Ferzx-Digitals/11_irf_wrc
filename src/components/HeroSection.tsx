...
const HeroSection = () => {
  ...
  useEffect(() => {
    const intervalId = window.setInterval(() => {
      setSlideIndex((prev) => {
        ...
        return (prev + 1) % content.heroSlides.length;
      });
    }, SLIDE_DURATION_MS);
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setPreviousIndex(null), TRANSITION_MS + 50);
    return () => window.clearTimeout(t);
  }, [previousIndex, slideIndex]);

  const layerClass = (i: number) => {
    if (i === slideIndex) return 'opacity-100 translate-y-0';
    if (i === previousIndex) return 'opacity-0 -translate-y-2 pointer-events-none';
      ...
      <div className="relative z-20 h-full">
        ...
        {content.heroSlides.map((slide, i) => (
          <div
            ...
            className={`absolute inset-0 flex flex-col items-center justify-center px-4 transition-[opacity,transform] duration-500 ease-out ${layerClass(i)}`}
          >
